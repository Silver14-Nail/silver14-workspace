...
// Response.json() returns Promise<unknown> under nodenext moduleResolution
...
export const getOrder = async (
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/orders/${orderId}?phone=${encodeURIComponent(phone)}`,
      {
        headers,
      },
    );
...
export const getProductInventory = async (
  productId: string,
): Promise<{ success: boolean; inventory?: ProductInventory; error?: string }> => {
  try {
...
export const updateProductInventory = async (
  data: UpdateInventoryRequest & { productName: string },
): Promise<{ success: boolean; inventory?: ProductInventory; error?: string }> => {
  try {
