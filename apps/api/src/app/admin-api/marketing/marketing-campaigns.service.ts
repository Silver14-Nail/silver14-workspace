  ...
  private async upsertTranslations(
    ...
    for (const t of translations) {
      const existing = await this.translationRepo.findOne({
        where: { campaignId, locale: t.locale },
      });

      if (existing) {
        if (t.eyebrow !== undefined) existing.eyebrow = t.eyebrow ?? null;
        if (t.title !== undefined) existing.title = t.title ?? null;
        if (t.subtitle !== undefined) existing.subtitle = t.subtitle ?? null;
        if (t.ctaLabel !== undefined) existing.ctaLabel = t.ctaLabel ?? null;
        if (t.secondaryCtaLabel !== undefined) existing.secondaryCtaLabel = t.secondaryCtaLabel ?? null;
        if (t.secondaryCtaUrl !== undefined) existing.secondaryCtaUrl = t.secondaryCtaUrl ?? null;
        if (t.isAutoGenerated !== undefined) existing.isAutoGenerated = t.isAutoGenerated ?? false;
