    ...
    <div className="p-6">
      ...
      <form onSubmit={handleSearch} className="mb-4 flex gap-2">
        ...
        >
          ...
          />
          {search && (
            <button type="button" onClick={() => { setSearch(''); router.push('/admin/supplies'); }}>
          ...
          <tbody className={`divide-y ${isDark ? 'divide-gray-800' : 'divide-[#F3F4F6]'}`}>
            {supplies.length === 0 ? (
              ...
              supplies.map((supply) => {
                ...
                return (
                  <tr key={supply.id} className={`transition-colors ${isDark ? 'hover:bg-gray-800' : 'hover:bg-[#F9FAFB]'}`}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-[#F3F4F6] flex items-center justify-center overflow-hidden">
                          {supply.images[0] ? (
                            <img src={supply.images[0].url} alt={supply.name} className="w-10 h-10 object-cover" />
                          ) : (
                            <Package className="w-5 h-5 text-[#9CA3AF]" />
                          )}
                        </div>
                        <div className="min-w-0">
                          <p className={`text-sm font-medium truncate ${isDark ? 'text-white' : 'text-[#111827]'}`}>
                            {supply.name}
                          </p>
                          {supply.description && (
                            <p className="text-xs text-[#9CA3AF] truncate max-w-[200px]">{supply.description}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      {supply.salePrice != null && (
                        <p className="text-xs text-red-500">${Number(supply.salePrice).toFixed(2)}</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-sm font-medium ${stockQty === 0 ? 'text-red-500' : stockQty <= 5 ? 'text-amber-500' : isDark ? 'text-green-400' : 'text-green-600'}`}>
                        {stockQty}
                      </span>
